...
class DnsimpleCertificate
  ...
  def initialize(token:, account_id:, domain:, common_name:)
  ...
  def private_key
    @certificate_private_key ||= client.certificates.certificate_private_key(account_id, domain, certificate.id).data.private_key
  end
  ...
  def certificate_for_common_name?(certificate)
    ...
    certificate.authority_identifier == "letsencrypt" &&
